...
if df is not None:
    ...
    with st.sidebar:
        ...
        # セッション状態の初期化
        sk = f'idx_{t_id}'
        rk = f'rec_{t_id}' # 収録中フラグ
        if sk not in st.session_state: st.session_state[sk] = 0
        if rk not in st.session_state: st.session_state[rk] = False # 初期は「収録中ではない」
        
        idx = st.session_state[sk]
        is_recording = st.session_state[rk]

        log_key = f'logs_{t_id}'
        if log_key not in st.session_state: st.session_state[log_key] = []

        # 1. 収録開始ボタン
        if not is_recording and idx < len(scn):
            if st.button("▶️ 収録開始", use_container_width=True, type="primary"):
                st.session_state[rk] = True
                st.rerun()
        elif is_recording:
            st.info("⏺️ 収録中...")

        # 2. 操作ボタン（is_recordingがFalseならdisabled=True）
        if idx < len(scn):
            curr = scn.iloc[idx]
            ...
            c1, c2 = st.columns(2)
            # 収録開始していない時はボタンを無効化 (disabled)
            with c1:
                u_active = (is_recording and curr['speaker']=="USER")
                if st.button("🙋 USER 終了", use_container_width=True, 
                             type="primary" if u_active else "secondary", 
                             disabled=not is_recording):
                    if curr['speaker']=="USER": add_log("USER", curr['turn_id'])
            with c2:
                s_active = (is_recording and curr['speaker']=="SYSTEM")
                if st.button("🤖 SYSTEM 終了", use_container_width=True, 
            ...
            c3, c4 = st.columns(2)
            with c3:
                if st.button("↩️ 戻る", use_container_width=True, disabled=not is_recording):
            ...
            with c4:
                if st.button("🔄 終了", use_container_width=True):
                    st.session_state[rk] = False # 終了時にフラグ解除
